...
export default {
  getAllParkingLotInfo({commit}){
    axios.get(`http://39.98.242.177:9090/parking-lots`)
      .then((response) => {
  ...
  getAllParkingOrder({commit}){
    var state = 'nobodydo'
    axios.get('http://39.98.242.177:9090/grab-order?state='+state)
      .then((response) => {
  ...
  getHistoryOrderByEmployeeId({state,commit}){
    axios.get('http://39.98.242.177:9090/login/order?employeeId='+JSON.parse(localStorage.getItem('token_key') || ' '))
      .then((response) => {
  ...
  login({commit}, userLogin) {
    axios.post(`http://39.98.242.177:9090/login`,userLogin)
      .then((response) => {
  ...
  finishOrderAndUpdateStatu({state,commit }, orderId){
    ...
    axios.put(`http://39.98.242.177:9090/Orders/${orderId}?stateMsg=${stateMsg}`)
      .then((response) => {
  ...
  grabOderThenUpdateState({state,commit},id){
    ...
    axios.put(`http://39.98.242.177:9090/Orders/${id}?stateMsg=${stateMsg}`)
      .then((response) => {
        state.orders = response.data
